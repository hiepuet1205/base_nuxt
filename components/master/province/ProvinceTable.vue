  ...
  <v-data-table :headers="headers" :items="items" sort-by="name" class="elevation-1">
    <template v-slot:top>
      <v-toolbar flat color="white">
        <v-toolbar-title>Province</v-toolbar-title>
...
import { getAllProvinces, getProvince, createProvince, updateProvince, deleteProvince } from '../../api/province'
export default ({
  data: () => ({
    dialog: false,
    headers: [
  ...
  methods: {
    ...
    async deleteItem(item) {
      ...
      if (confirm('Are you sure you want to delete this item?')) {
        ...
        if (await deleteProvince(item.id)) {
          this.items.splice(index, 1)
        }
      }
    },
    ...
    async save() {
      if (this.editedIndex > -1) {
        const data = await updateProvince(this.editedItem.id, this.editedItem);
        this.editedItem = data;
        this.items[this.editedIndex] = this.editedItem;
      } else {
        const data = await createProvince(this.editedItem)
        this.editedItem = data;
        this.items[this.editedIndex] = this.editedItem;
